...
//        if ((data || '').indexOf('<html') !="-1)" {="" pjax.options.callback="" &&="" pjax.options.callback.call(pjax.options.element,="" type="" :="" 'error'="" });="" location.href="pjax.options.url;" return="" false;="" }="" var="" title="pjax.options.title" ||="" "",="" el;="" if="" (pjax.options.element)="" el="$(pjax.options.element);" el.text();="" matches="data.match(/<title">(.*?)<\ title="">/);
        if (matches) {
            title = matches[1];
        }
        if (title) {
...
$.extend($.pjax.showFx,{
    custom: function(data, callback, isCached){
        ...
        while(depth > 0) {
            ...
            //count occurrences
            i = 0;
            pos = temp.indexOf("<div"); while="" (pos="" !="-1)" {="" i++;="" pos="temp.indexOf("<div"," +="" 1);="" }="" end="" count="" depth="depth+i-1;" output="output+data.split('</div">')[0] + '';
